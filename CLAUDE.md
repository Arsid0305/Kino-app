...
**Что работает:**
- Vercel фронт-деплой (отдельное приложение, не через Actions) — push в main = фронт задеплоен, preview на PR тоже работает
- Прямые REST-мержи через MCP (`merge_pull_request`) — работают, потому что это не workflow
...
2. **После мержа PR, где менялись `supabase/functions/**`** — сразу: «Edge function изменена, автодеплой не работает. Задеплой вручную: Supabase Dashboard → Edge Functions → `<имя>` → Deploy. Иначе прод не обновится».
